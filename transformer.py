...
class Net(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, padding_idx):
        ...
        self.gru = nn.GRU(input_size = embedding_size, 
        					...
        					num_layers=3)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model = hidden_size, nhead = 8) #d_model = hidden_size must be divisible by nhead
        self.trans_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=1)  #If we want many encoder layers
        self.fc1 = nn.Linear(hidden_size + 1, 50) #try different number of hidden units?
    ...
    def forward(self, batch, tag, hidden):
    	...
    	output = self.trans_encoder(output) #transformer encoder layer, maybe I should encode the hidden state too? nah
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(3, batch_size, self.hidden_size, device=device) #change first dimension for number of hidden layers of gru
